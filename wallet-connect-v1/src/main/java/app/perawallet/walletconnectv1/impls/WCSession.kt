...
import app.perawallet.walletconnectv1.Session.Transport.Builder
import app.perawallet.walletconnectv1.Session.Transport.Message
import app.perawallet.walletconnectv1.Session.Transport.Status
import app.perawallet.walletconnectv1.nullOnThrow
import app.perawallet.walletconnectv1.types.extractSessionParams
import app.perawallet.walletconnectv1.types.intoMap
...
class WCSession(
    ...
    override fun offer() {
        if (transport.connect()) {
            ...
            send(
                Session.MethodCall.SessionRequest(requestId, clientData, chainId),
    ...
    private fun handleStatus(status: Status) {
        when (status) {
            ...
            is Status.Disconnected -> Session.Status.Disconnected(status.isSessionDeletionNeeded)
            is Status.Error -> Session.Status.Error(Session.TransportError(status.throwable))
        }
        propagateToCallbacks {
            onStatus(
                when (status) {
                    Status.Connected -> Session.Status.Connected(clientData.id)
                    is Status.Disconnected -> Session.Status.Disconnected(status.isSessionDeletionNeeded)
                    is Status.Error -> Session.Status.Error(Session.TransportError(status.throwable))
                }
            )
    ...
    private fun handleMessage(message: Message) {
        ...
        when (data) {
            ...
            is Session.MethodCall.Custom -> {
                // TODO handle custom method calls
            }
        }
    ...
    private fun send(
        ...
        transport.send(Message(topic, "pub", payload))
        return true
    }
