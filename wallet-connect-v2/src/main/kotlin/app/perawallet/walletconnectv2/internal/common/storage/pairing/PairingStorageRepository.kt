...
import app.perawallet.walletconnectv2.sdk.storage.`data`.dao.PairingQueries
...
class PairingStorageRepository(private val pairingQueries: PairingQueries) :
    PairingStorageRepositoryInterface {
    ...
    override fun hasTopic(topic: Topic): Boolean =
        pairingQueries.hasTopic(topic = topic.value).executeAsOneOrNull() != null

    @Throws(SQLiteException::class)
    override suspend fun getListOfPairings(): List<Pairing> =
        pairingQueries.getListOfPairing(mapper = this::toPairing).awaitAsList()

    @Throws(SQLiteException::class)
    override suspend fun getListOfPairingsWithoutRequestReceived(): List<Pairing> =
        pairingQueries.getListOfPairingsWithoutRequestReceived(mapper = this::toPairing)
            .awaitAsList()
    ...
    @Throws(SQLiteException::class)
    override fun updateExpiry(topic: Topic, expiry: Expiry): QueryResult<Long> {
        return pairingQueries.updateOrAbortExpiry(expiry = expiry.seconds, topic = topic.value)
    }

    @Throws(SQLiteException::class)
    override fun getPairingOrNullByTopic(topic: Topic): Pairing? =
        pairingQueries.getPairingByTopic(topic = topic.value, mapper = this::toPairing)
            .executeAsOneOrNull()
